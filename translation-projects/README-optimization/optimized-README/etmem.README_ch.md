...
| sleep              | 每个内存扫描+操作的大周期之间的时间间隔                      | 是                           | 是         | 1~1200                                                       | sleep=10 //每次大周期之间间隔10s                             |
...
| node_pair          | cslide engine的配置项，声明系统中AEP和DRAM的node pair        | engine为cslide时必须配置     | 是         | 成对配置AEP和DRAM的node号，AEP和DRAM之间用逗号隔开，每队pair之间用分号隔开 | node_pair=2,0;3,1                                            |
...
| vm_flags           | engine为cslide的task配置项，通过指定flag扫描vma，不配置此项时扫描则不会区分 | engine为cslide时必须配置     | 是         | 当前只支持ht                                                 | vm_flags=ht //扫描flags为ht（大页）的vma内存                 |
